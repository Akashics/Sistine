...
module.exports = class extends Command {
	...
	async run(message, [command]) {
		return message.reply('All of my commands can be found on my homepage: `https://Senko-san.moe/`');
		if (command) {
			const info = [
				`❴ ${command.name} ❵ `,
		...
		for (let cat = 0; cat < categories.length; cat++) {
			if (categories[cat] === 'NSFW' && message.channel.type !== 'dm' && !message.channel.nsfw && !message.flags.nsfw) continue;
			helpMessage.push(`**${categories[cat]} Commands**:`, '```asciidoc');
			const subCategories = Object.keys(help[categories[cat]]);
			for (let subCat = 0; subCat < subCategories.length; subCat++) {
				helpMessage.push(`${subCategories[subCat] !== 'General' ? `❴ ${subCategories[subCat]} ❵` : ''}`, `${help[categories[cat]][subCategories[subCat]].join('\n')}\n`);
			}
			helpMessage.push('```', '\u200b');
		}
		...
		return message.author.send(helpMessage, { split: { char: '\u200b' } })
			...
			.catch((err) => { console.log(err); if (message.channel.type !== 'dm') message.sendLocale('COMMAND_HELP_NODM'); });
	}
