...
module.exports = class LoveShip extends Command {
	...
	async run(msg, [user, user2]) {
		const mssg = await msg.send('<a:loading:457038953439166464> Waiting for resources to load.');
		...
		const { body } = await snek.post(`https://api.weeb.sh/auto-image/love-ship`)
			.set('Authorization', `Wolke ${this.client.config.weebservices}`)
			.set('User-Agent', 'Senko-san/1.0.0-dev')
