...
module.exports = class extends MusicCommand {
	...
	async run(msg) {
		...
		if (!music.voiceChannel) throw `｢ **Error** ｣ I am not connected to a voice channel.`;
		...
		if (vote && vote.count >= 1) {
			if (vote.users.some(user => user === msg.author.id)) return msg.reply(`｢ **Error** ｣ You've already voted to skip this song.`);
	...
	skip(guild) {
		...
		guild.music.skip();
		return `Skipped: **${song ? song.title : 'N/A'}**`;
	}
	...
	setTimeout(textChannel, vote) {
		...
		vote.timeout = setTimeout(() => {
			...
			textChannel.send(`The vote to skip the current song has ended.`);
		}, time);
